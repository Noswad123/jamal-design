      ...
      return (
        <Container>
            ...
            {Projects.map(
                (x,index)=>{
                    return(
                        <Project key={index} id={index} img={x.imgUrl} openPopUp={this.openPopUp.bind(this)} name={x.name} description={x.description}  status={x.status} url={x.myUrl} />
                    )
                }
            )}
            </Wrapper>

      <Link to="/contactme"><button>Contact Me</button></Link>
        </Container>
  )
}
}